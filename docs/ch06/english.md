...
<div class="case-card">
<span class="case-label">📌 Coze实战案例：英语四级单词复习智能体</span>

### 背景

使用Coze搭建交互式英语单词复习智能体，对话就能生成单词卡片，支持"跳过"和"已学习"按钮操作。

**核心功能**：
- **随机生成单词卡片**：包含单词、音标、释义、例句
- **交互操作**："跳过"或"已学习"按钮，点击继续下一张
- **学习记录**：数据库记录每个单词的学习次数
- **语音合成**：生成英文例句的音频
...
| image_url | VARCHAR | 单词卡片图片URL |
| read_count | INT | 学习次数 |

**智能体提示词**：
```markdown
# 角色
你是一个英语四级单词复习智能助手，能够辅助用户高效复习
英语四级单词。

## 技能
### 技能 1: 处理特定指令
1. 输入"我要学习"时，调用word_card_generator工作流
2. 当用户输入"跳过"或"已学习"时，调用对应工作流进行处理
...
- 图片区域显示单词卡片（9:16比例）
- 音频区域播放例句发音
- 按钮："跳过"（发"跳过"消息）+ "已学习"（发"已学习"消息）
- 显示已学习总次数（read_count > 0时）
...
> 💡 **关键**：数据库+工作流+智能体卡片的组合，实现了完整的交互式学习体验。
